...
from ..theme import PRESET_BUTTON_STYLESHEET
...
class DownloadsPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        # Auto-add checkbox
        self._auto_add_check = QCheckBox(self.tr("Adicionar ao mapa ao concluir"))
        self._auto_add_check.setChecked(True)
        layout.addWidget(self._auto_add_check)

        # Empty state
        self._empty_label = QLabel(self.tr("Nenhum download em andamento"))
        self._empty_label.setAlignment(Qt.AlignCenter)
        self._empty_label.setStyleSheet("color: #bdc3c7; font-size: 12px; padding: 20px;")
    ...
    def _on_download_started(self, item_id):
        ...
        self._table.setItem(row, 3, QTableWidgetItem(self.tr("Baixando...")))

    def _on_download_progress(self, item_id, percent):
        row = self._rows.get(item_id)
        if row is None:
            return
        progress = self._table.cellWidget(row, 2)
    ...
    def _on_download_completed(self, item_id, file_path):
        ...
        self._table.setItem(row, 3, QTableWidgetItem(self.tr("Concluido")))
    ...
    def _on_download_failed(self, item_id, error):
        ...
        if row is None:
            return
        self._table.setItem(
            row, 3, QTableWidgetItem(self.tr("Erro: {msg}").format(msg=error[:30]))
        )
        self._update_empty_state()
    ...
    def _update_empty_state(self):
        active = self._download_controller.active_downloads
        self._empty_label.setVisible(len(active) == 0 and self._table.rowCount() == 0)
